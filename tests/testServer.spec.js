...
describe('4 - Criar um túnel através do Ngrok', () => {
  it.skip('Será validado se os comandos estão dentro do arquivo instruction.json', async () => {
    ...
    expect(instructionJson.linkSetup).toContain(
      'https://dashboard.ngrok.com/get-started/setup',
    );
    expect(instructionJson.commandExtractNgrok).toContain(
    ...
    expect(instructionJson.commandStart).toContain(
...
describe('10 - Responder o IP do client', () => {
  it.skip('Será validado que ao acessar a url sera possível visualizar o ip do client', async () => {
    ...
    const url = await page.$$eval('a[target="_blank"]', (nodes) =>
    ...
    const textIp = await newPage.$$eval(dataTestid('ip'), (nodes) =>
      nodes.map((n) => n.innerText),
    );
...
describe('11 - Responder informações extraídas através do IP do client', () => {
  it.skip('Será validado que as informações da localização do cliente serão exibidas na tela', async () => {
    ...
    const textCity = await newPage.$$eval(dataTestid('city'), (nodes) =>
      nodes.map((n) => n.innerText),
    );
    await newPage.waitForSelector(dataTestid('postal_code'));
    const textPostaCode = await newPage.$$eval(
      dataTestid('postal_code'),
      (nodes) => nodes.map((n) => n.innerText),
    );
    await newPage.waitForSelector(dataTestid('region'));
    const textRegion = await newPage.$$eval(dataTestid('region'), (nodes) =>
      nodes.map((n) => n.innerText),
    );
    await newPage.waitForSelector(dataTestid('country'));
    const textCountry = await newPage.$$eval(dataTestid('country'), (nodes) =>
      nodes.map((n) => n.innerText),
    );
    await newPage.waitForSelector(dataTestid('company'));
    const textCompany = await newPage.$$eval(dataTestid('company'), (nodes) =>
      nodes.map((n) => n.innerText),
    );
...
describe('12 - Responder dados do dispositivo (client)', () => {
  it.skip('Será validado se que ao acessar a tela listou os dados do dispositivo', async () => {
    ...
    const deviceText = await newPage.$$eval(dataTestid('device'), (nodes) =>
      nodes.map((n) => n.innerText),
    );
...
describe('13 - Responder a request com os resources do Server', () => {
  it.skip('Validar se acessar o site vai listar as informações do sistema', async () => {
    ...
    const textArch = await newPage.$$eval(dataTestid('arch'), (nodes) =>
      nodes.map((n) => n.innerText),
    );
    await newPage.waitForSelector(dataTestid('cpu'));
    const textCpu = await newPage.$$eval(dataTestid('cpu'), (nodes) =>
      nodes.map((n) => n.innerText),
    );
    await newPage.waitForSelector(dataTestid('memory'));
    const textMemory = await newPage.$$eval(dataTestid('memory'), (nodes) =>
      nodes.map((n) => n.innerText),
    );
